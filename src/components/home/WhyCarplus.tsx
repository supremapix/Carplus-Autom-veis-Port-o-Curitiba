      ...
      <Container className="relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {pillars.map((pillar, index) => (
            <div
              key={index}
              className="bg-white hover:bg-[#FAFAFA] text-black border-2 border-black rounded-2xl p-7 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-black/40 flex flex-col justify-between group relative overflow-hidden shadow-lg"
            >
              {/* Linha de acento sutil no topo do card ao passar o mouse */}
              <div className="absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r from-transparent via-[#F59C00] to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />

              <div>
                <div className="flex items-center justify-between mb-5">
                  <div className="w-14 h-14 rounded-2xl bg-black border border-black flex items-center justify-center transition-all group-hover:scale-105 shadow-md">
                    {pillar.icon}
                  </div>
                  <span className="text-[10px] font-display font-bold uppercase tracking-[0.16em] text-black bg-[#F59C00]/20 px-2.5 py-1 rounded-full border border-[#F59C00]/40">
                    {pillar.tag}
                  </span>
                </div>

                <h3 className="font-display font-bold text-xl uppercase text-black tracking-wide mb-2.5 leading-snug group-hover:text-[#F59C00] transition-colors">
                  {pillar.title}
                </h3>
